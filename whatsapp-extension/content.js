...
(function () {
  ...
  function inserirArquivo(file) {
    ...
    // 1) Campo de DOCUMENTO, o caminho limpo.
    ...
    // 2) Colar na caixa de texto.
    //
    // Em 18/09 eu tirei este caminho porque imagem colada vira FOTO, e o
    // WhatsApp comprime - as regras de pet chegavam ilegiveis. So que sem ele,
    // quando o campo de documento nao aparece, nao ia NADA (23/09). Agora os
    // anexos sao todos PDF, e PDF colado o WhatsApp trata como documento, sem
    // compressao. Por seguranca, imagem continua barrada aqui.
  ...
  async function anexar(a) {
    ...
    if (via === 'documento') {
      status('Anexado como documento (sem compressão) — confira e envie.');
    } else if (via === 'colado') {
      status('Anexado (colado como PDF, sem compressão) — confira e envie.');
    } else {
      status(
        'Não consegui anexar. Clique no clipe 📎 → Documento e escolha o arquivo.' +
          (abriu ? '' : ' (não achei o botão do clipe nesta tela)'),
        true,
      );
    }
  }
